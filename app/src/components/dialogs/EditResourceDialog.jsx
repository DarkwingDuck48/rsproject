 ...
 */
export default function EditResourceDialog({ open, resource, onClose, onSaved }) {
  ...
  return (
    <Modal open={open} title={`Ресурс: ${resource?.name ?? ""}`} footer={null} destroyOnHidden>
      <Form
        layout="vertical"
        initialValues={{
          name: resource?.name,
      ...
      >
        ...
        <Form.Item name="rate" label="Ставка" rules={[{ required: true, message: "Укажите ставку" }]}>
          <InputNumber min={0} precision={2} placeholder="Ставка" style={{ width: "100%" }} />
        </Form.Item>
        <Form.Item name="rate_measure" label="Мера ставки" rules={[{ required: true, message: "Выберите меру ставки" }]}>
